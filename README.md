...
##### 백승하(조원1) : FrontEnd,BackEnd (FrontEnd(매장결제 승인, 입점회원가입,로그인,마이페이지) , BackEnd(입점회원가입,회원정보수정,마이페이지 BackEnd)
##### 정세인(조원2) : FrontEnd,BackEnd (FrontEnd(웹 기획, 메인페이지 구현,주문내) , BackEnd(로그인,주문내역,메뉴상세 BackEnd)
<br/>
